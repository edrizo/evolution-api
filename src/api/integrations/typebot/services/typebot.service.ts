...
export class TypebotService {
  ...
  public async startTypebot(instance: InstanceDto, data: any) {
    ...
    if (startSession) {
      ...
      try {
        ...
        let reqData: {};
        if (version === 'latest') {
          urlToCall = `${data.url}/api/v1/typebots/${data.typebot}/startChat`;

          reqData = {
            prefilledVariables: prefilledVariables,
          };
        } else {
          urlToCall = `${data.url}/api/v1/sendMessage`;
        ...
        const request = await axios.post(urlToCall, reqData);
    ...
    async function processMessages(instance, messages, input, clientSideActions, eventEmitter, applyFormatting) {
      for (const message of messages) {
        if (message.type === 'text') {
          ...
          // Remover marcadores duplicados, sublinhado etc. se necessário
   ...
   * Encerra sessão se msg.key?.fromMe === true
   * ------------------------------------------
   */
  public async sendTypebot(instance: InstanceDto, remoteJid: string, msg: MessageRaw) {
    // 1. Se a mensagem partiu de você (agente/administrador), encerra a sessão de imediato
    if (msg.key?.fromMe) {
      this.logger.verbose(`Encerrando sessão, pois a mensagem veio do agente (fromMe = true). RemoteJid: ${remoteJid}`);
    ...
    // 2. Se não foi você quem enviou, segue a lógica normal do Typebot
    const findTypebot = await this.find(instance);
    ...
    try {
      // 2A. Verifica expiração se a sessão existir
      if (session && expire && expire > 0) {
        ...
        if (diffInMinutes > expire) {
          ...
          // Cria nova sessão e envia a primeira mensagem de boas-vindas
          ...
          // Se o Typebot não retornou mensagens e chegou um conteúdo do usuário
          if (data.messages.length === 0) {
            ...
            // Envia a mensagem do usuário pro Typebot (continueChat)
            try {
              const version = this.configService.get<Typebot>('TYPEBOT').API_VERSION;
      ...
      // 2B. Se não há sessão ainda, cria uma nova
      if (!session) {
        const data = await this.createNewSession(instance, {
          enabled: findTypebot.enabled,
          url: url,
        ...
        // Se o Typebot não retornou mensagens
        if (data.messages.length === 0) {
          if (!content) {
            if (unknown_message) {
              this.waMonitor.waInstances[instance.instanceName].textMessage({
      ...
      // 2C. Se a sessão existe e está aberta, atualiza updateAt
      ...
      // 2D. Se não veio conteúdo, manda unknown_message
      if (!content) {
        if (unknown_message) {
          this.waMonitor.waInstances[instance.instanceName].textMessage({
            number: remoteJid.split('@')[0],
      ...
      // 2E. Caso contrário, envia a mensagem do usuário ao Typebot
      const version = this.configService.get<Typebot>('TYPEBOT').API_VERSION;
      let urlTypebot: string;
      let reqData: {};
      if (version === 'latest') {
        // sessionId é do tipo "1234567890-<idDoTypebot>"
